...
- **Feed (2026-08-25, GLG request):** RSS 2.0 at `/feed.xml`, generated by `scripts/genfeed`
  ...
  **Run it *after* committing the brick, then commit the feed** — a first-add date does
  not exist until the brick is committed, and before that `genfeed` silently falls back
  to `new Date()`, which is how the tenth brick shipped a pubDate two seconds off its own
  commit (measured 2026-09-12). Discovery `<link rel="alternate">` on
